...
function addToChunkHistory(chunkKey, timestamp, transactionId, changes, address) {
    ...
    // If so, merge the changes instead of skipping
...
// Test 5: Same transaction ID with different changes should merge changes
runTest('Same transaction ID merges changes instead of ignoring', () => {
    const chunkKey = 'world:0:0';
    
    // First update with some changes
    applyChunkUpdate(chunkKey, 1000, 'tx1', [
        { x: 5, y: 10, z: 5, b: 1 } // Stone at position A
    ]);
    
    // Same transaction ID but different position should be merged
    applyChunkUpdate(chunkKey, 1000, 'tx1', [
        { x: 6, y: 10, z: 5, b: 2 } // Grass at position B
    ]);
    
    const chunk = mockChunks.get(chunkKey);
    // Both changes should be applied
    assertEqual(chunk.get(5, 10, 5), 1, 'Position A should be Stone');
    assertEqual(chunk.get(6, 10, 5), 2, 'Position B should be Grass');
    
    const history = IPFS_CHUNK_MESSAGE_HISTORY.get(chunkKey);
    assertEqual(history.length, 1, 'History should have only 1 entry');
    assertEqual(history[0].changes.length, 2, 'Entry should have 2 changes merged');
});
