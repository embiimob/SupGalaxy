        ...
        var VERSION = 'SupGalaxy v0.3.3-beta';
...
self.onmessage = async function(e) {
        ...
        function createBlockTexture(seed, blockId) {
            ...
            const secondaryColor = new THREE.Color().setHSL(rnd(), 0.5 + rnd() * 0.3, 0.2 + rnd() * 0.3);
        ...
        function initThree() {
            ...
            var dir = new THREE.DirectionalLight(0xffffff, 0.75); // Reduced intensity
            dir.position.set(100, 200, 100);
            scene.add(dir);
            scene.add(new THREE.AmbientLight(0xffffff, 0.25)); // Reduced intensity
        ...
        function toggleInventory() {
            ...
            if (!isVisible) {
                ...
                selectedInventoryIndex = -1; // Clear selection when closing
            }
        }
        ...
        function createInventorySlot(index) {
            ...
            slot.addEventListener('click', function() {
                ...
                if (selectedInventoryIndex === -1) {
                    // Nothing selected, so select this slot
                    selectedInventoryIndex = clickedIndex;
                } else {
                    // Something is already selected, so swap items
                ...
                updateInventoryUI();
                updateHotbarUI(); // Also update the main game hotbar
            });
        ...
        function onPointerDown(e) {
            ...
            // Add player hit detection
            var playerHitAvatars = [];
            for (const [username, avatar] of playerAvatars.entries()) {
                if (username !== userName) {
                    const intersect = raycaster.intersectObject(avatar, true)[0];
                    if (intersect) {
                        playerHitAvatars.push({ username, intersect });
                    }
                }
            }

            playerHitAvatars.sort((a, b) => a.intersect.distance - b.intersect.distance);

            if (playerHitAvatars.length > 0) {
                const hitPlayer = playerHitAvatars[0];
                const message = JSON.stringify({
                    type: 'player_hit',
                    target: hitPlayer.username,
                    username: userName
                });

                if (isHost) {
                    // Host hitting a client, process it directly
                    handlePlayerHit(JSON.parse(message));
                } else {
                    // Client hitting another player, send to host for validation
                    ...
                    // Client gets optimistic feedback
                ...
                player.vx -= backDir.x * recoilStrength;
                player.vz -= backDir.z * recoilStrength;
            ...
            var wx = Math.floor(p.x - norm.x * 0.5);
            var wy = Math.floor(p.y - norm.y * 0.5);
            var wz = Math.floor(p.z - norm.z * 0.5);
            if (e.button === 0) {
                animateAttack();
                removeBlockAt(wx, wy, wz);
                safePlayAudio(soundBreak);
            } else if (e.button === 2) {
                var placeX = Math.floor(p.x + norm.x * 0.5);
                var placeY = Math.floor(p.y + norm.y * 0.5);
                var placeZ = Math.floor(p.z + norm.z * 0.5);
        ...
        function placeBlockAt(wx, wy, wz, bid) {
            ...
            if (BLOCKS[bid] && BLOCKS[bid].light) {
                ...
                light.position.set(wx, wy + 0.5, wz);
                scene.add(light);
                torchLights.set(`${wx},${wy},${wz}`, light);

                var particles = createFlameParticles(wx, wy + 0.5, wz);
                scene.add(particles);
                torchParticles.set(`${wx},${wy},${wz}`, particles);
            }
        }
        ...
        function setupDataChannel(dc, user) {
            ...
            dc.onmessage = e => {
                ...
                try {
                    ...
                    // Avatar is now created inside the state_update/player_move handlers
                    // to ensure initial yaw is available.
                    ...
                    switch (data.type) {
                            ...
                            if (!isHost) {
                                for (const playerData of data.players) {
                                    ...
                                    if (!data.timestamp || data.timestamp > (userState.lastTimestamp || 0)) {
                                        ...
                                        // Only trigger a new animation if the start time from the host is new
                                        if (playerData.attackStartTime && playerData.attackStartTime !== userState.attackStartTime) {
                                            userState.attackStartTime = playerData.attackStartTime; // Store the host's timestamp
                                            userState.localAnimStartTime = performance.now(); // Start animation based on local time
                                        }
                                    }
                                }
                            }
                        ...
                        case 'player_respawn':
                            ...
                            // Recreate the remote avatar to ensure it's reset correctly
                        ...
                        case 'block_change':
                            ...
                            // Handle light removal for the previous block
                            ...
                            // Handle light creation for the new block
                        ...
                        case 'add_score':
                            ...
                            break;
                        case 'player_attack':
                            // Received from a client. Host processes this.
                            if (isHost) {
                                ...
                                if (attackerState) {
                                    ...
                                    attackerState.localAnimStartTime = now; // Trigger animation locally on host
                                }
                            }
                            break;
                        case 'player_death':
                            // Received from a peer. Animate remote player's death.
                            ...
                            if (userPositions[deadPlayerUsername]) {
                                ...
                                if (deadPlayerAvatar) {
                                    deadPlayerAvatar.visible = true; // Ensure avatar is visible for animation
                                }
                            }
                            break;
                    }
        ...
        function activateHost() {
            if (!isHost) {
                isHost = true;
                console.log('[WebRTC] Hosting activated.');
        ...
        async function acceptPendingOffers() {
            ...
            for (const checkbox of checkboxes) {
                ...
                try {
                    ...
                    pc.oniceconnectionstatechange = () => console.log(`[WebRTC] ICE state change for ${clientUser}: ${pc.iceConnectionState}`);
                    peers.set(clientUser, { pc, dc: null, address: offer.profile && offer.profile.Creators ? offer.profile.Creators[0] : null });
                    pc.ondatachannel = (e) => {
                        const dc = e.channel;
        ...
        document.addEventListener('DOMContentLoaded', function () {
            try {
                ...
                document.getElementById('switchWorldBtn').addEventListener('click', switchWorld);
