...
self.onmessage = async function(e) {
        ...
        function createVolcanoSmoke(x, y, z) {
        ...
        function triggerCalderaEruption(volcano, eruptionSeed) {
            ...
            for (let i = 0; i < numBoulders; i++) {
                ...
                const size = 0.5 + rnd() * 2.0;
                const blockId = rnd() > 0.3 ? 4 : 110;
                ...
                const targetAngle = rnd() * Math.PI * 2;
                const distance = 16 + rnd() * 32;
                const speed = 20 + rnd() * 15;
                const vx = Math.cos(targetAngle) * speed * (distance / 48);
                const vz = Math.sin(targetAngle) * speed * (distance / 48);
                const vy = 25 + rnd() * 20;
        ...
        function findAndManageVolcanoes() {
            ...
            // Remove volcanoes that are too far away
            activeVolcanoes = activeVolcanoes.filter(v => {
                const isClose = Math.hypot(v.x - player.x, v.z - player.z) < SCAN_RADIUS * 2;
                if (!isClose) {
        ...
        function gameLoop(now) {
            ...
            if (deathScreenShown) {
                ...
                // Update volcano smoke
                for (const volcano of activeVolcanoes) {
                    const positions = volcano.smokeParticles.geometry.attributes.position.array;
                    ...
                    for (let i = 0; i < velocities.length; i++) {
                        ...
                        if (velocities[i].life <= 0) {
                            ...
                            positions[i * 3] = volcano.x + Math.cos(angle) * r;
                            positions[i * 3 + 1] = volcano.y + (Math.random() - 0.5) * 2;
                            positions[i * 3 + 2] = volcano.z + Math.sin(angle) * r;
                            velocities[i].life = Math.random() * 8 + 4;
                        }
                    }
                    volcano.smokeParticles.geometry.attributes.position.needsUpdate = true;
                }
